...
<head>
    <meta charset="utf-8">
    <title>Event Payload - Documentation</title>
...
<p>ChatEngine event payloads are augmented with additional information supplied by the framework. Most of the time these are <code>payload.user</code> and ```payload.chat.</p>
<p>The property <code>payload.chat</code> is the <a href="Chat.html">Chat</a> that event was broadcast broadcast on, and the <code>payload.user</code> is the <a href="User.html">User</a> that broadcast the message.</p>
<p>The property <code>payload.user</code> is the <a href="User.html">User</a> that is responsible for calling <a href="Chat.html#emit">Chat#emit</a>.</p>
